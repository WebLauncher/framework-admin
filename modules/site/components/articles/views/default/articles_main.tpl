{if $p.actions[0]==""}
	...
	<button type="button" class="floatleft"  onclick="admin_go_to('{$current}?article={bind table=$p.tables.tbl_articles get_field='parent_id'}{$p.session.articles.parent_id}{/bind}')"><b class="icon icon-arrow_left"></b><span>{tr tags="buttons"}Back{/tr}</span></button>
	{/if}	
	{if $p.subquery[2]=="news" || $p.subquery[2]=="events" || $p.subquery[2]=="newpage"}
	<button type="button" class="floatleft" onclick="go_to('{$root_component}?a=add:{$p.session.articles.parent_id|default:0}')"><b class="icon icon-plus"></b><span>New article</span></button>
	{/if}
	</div>
	<div class="clearfix folderview">
		...
		<div id="folder_{$f.id}" class="floatleft folder" onmouseover="$('#selected_folder .content').html($('#folder_{$f.id} .description').html());">
			<div class="wrapper">
				...
				<div class="actions clearfix">
					<a href="{$current}?a=delete:{$f.id}" class="floatleft" onclick="return confirm('Are you sure you want to delete this?');" title="Delete directory"><img src="{$skin_images}newicons/trash_16.png"></a>
				...
				<div class="description">
					...
					<a href="{$current}?a=edit_folder:{$f.id}">edit</a>&nbsp;<a href="{$current}?a=delete:{$f.id}" onclick="return confirm('Are you sure you want to delete this?');">delete</a>			
				</div>
			</div>
		</div>
		{/foreach}
		...
		<div id="file_{$f.id}" class="floatleft file" onmouseover="$('#selected_folder .content').html($('#file_{$f.id} .description').html());">
			<div class="wrapper">
				<a href="{$current}?a=edit:{$f.id}" class="clearfix">
					<div class="image">
						<div class="subicon"><img src="{$skin_images}languges/{$l.code|lower}.png" alt="{$l.valoare}"/></div>
					</div>
					<div class="title">
						{$f.title}
					</div>
				...
				<div class="actions clearfix">				
					<a href="{$current}?a=edit:{$f.id}" class="floatleft" title="Edit article"><img src="{$skin_images}newicons/pencil_16.png"></a>
					{if !$f.is_translation}<a href="#" onclick="admin_go_to('{$current}?article={$f.id}');" class="floatleft" title="View translations"><img src="{$skin_images}newicons/briefcase_16.png"></a>{/if}
					
				</div>
				<div class="description">
					<a href="{$current}?a=edit:{$f.id}"><strong>{$f.title}</strong></a> (<a href="{$current}?a=edit:{$f.id}">edit</a>)<br/>
					<small>Language: <img src="{$skin_images}languages/{$f.language.code|lower}.png" alt="{$f.language.valoare}"/></small><br/>
					{if $f.image_id}
					<div>
					...
					<small>Translations:
						{foreach item=tran from=$f.translations} 
							<a href="{$current}?a=edit:{$tran.id}" title="{tr}Edit translation{/tr}">{bind table=$p.tables.tbl_locales get_field="code" assign="l_code"}{$tran.language_id}{/bind}<img src="{$skin_images}languages/{$l_code|lower}.png" alt="{$l_code}"/></a>
						{/foreach}
					</small><br/>
					{/if}
				</div>
		...
		<div id="file_{$folder.id}_{$f}" class="floatleft file" onmouseover="$('#selected_folder .content').html($('#file_{$folder.id}_{$f} .description').html());">
			<a href="{$current}?a=add_translation_save:{$folder.id}&language_id={$f}" class="clearfix">
				<div class="image">
		...
		<div class="floatright details">
			<div class="panel">			
				...
				<div class="content">
					<strong>{$folder.title}</strong> {if $folder.id}(<a href="{$current}?a=edit_folder:{$folder.id}">edit</a>){/if}<br/>
			...
			<div class="panel" id="selected_folder">			
				<div class="title">{if $folder.is_directory}{tr}Selected directory/article{/tr}{else}{tr}Translation{/tr}{/if}</div>
				<div class="content">
					None
				</div>
			</div>
