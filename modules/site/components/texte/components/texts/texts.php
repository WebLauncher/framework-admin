...
class texts_page extends Page
{
    ...
    function action_edit($id, $language_id = "")
    {
        ...
        if ($language_id != "") {
            ...
            $text['language'] = $this->models->languages->get($text['language_id']);
            $text['language']['image'] = $this->models->images->get($text['language']['image_id']);
            $text['text'] = $this->models->texts->get($text['text_id']);
        } else {
                ...
                "Modificare text" => array(
                    "fields" => array(
                        "Actiunea:" => array(
                            "name" => "a",
                            "type" => "hidden",
                            "value" => "save:" . $id . ($language_id ? ":" . $language_id : "")
                        ),
                        "Limba hidden:" => array(
                            "name" => "language_id",
                            "type" => "hidden",
                            "value" => isset_or($text['language_id'])
                        ),
                        "Textul original:" => array(
                            "name" => "",
                            "type" => "none",
                            "value" => isset_or($text['query'], isset_or($text['text']['query']))
                        ),
                        "Textul curent:" => array(
                            "name" => "",
                            "type" => "none",
                        ...
                        "Limba:" => array(
                            "name" => "",
                            "type" => "image",
                            "value" => isset_or($text['language']['valoare']),
                            "url" => isset_or($text['language']['image']['current_http_path'])
                        ),
    ...
    function action_save($id, $language_id = "")
    {
                ...
                "Traducere text" => array(
                    "fields" => array(
                        ...
                        "Actiunea:" => array(
                            "name" => "a",
                            "type" => "hidden",
                            "value" => "save:" . $text['id'] . ($language_id ? $language_id : "")
                        ),
                        "Textul original:" => array(
                            'description' => 'Textul initial care apare in codul siteului.',
                            "name" => "",
                            "type" => "none",
                            "value" => isset_or($text['query'])
                        ),
                        "Textul curent:" => array(
                            'description' => 'Textul ce se afiseaza acum pe site.',
                            "name" => "",
                            "type" => "none",
                            "value" => $text['value']
                        ),
        ...
        if ($data) {
            $this->table->header = array(
                ...
                array(
                    ...
                    'col' => 'languages',
                    'array' => 1,
                    'eval' => '<a href="{$current}?a=edit:{$o.id.value}:{$obj.id}" title="{tr}Editeaza{/tr}">{image}{$obj.image_id}{/image}</a>',
